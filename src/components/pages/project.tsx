...
export default function Project({ image, heading, title, projectContent ,link }: any) {
    return (
        <div className='w-full h-[219px] bg-secondary rounded-xl flex mb-5'>
            <div className='w-[20%] h-full flex justify-center items-center'>
                <img className='rounded-full w-[90%] h-[90%]' src={image} alt="" />
            </div>
            <div className='w-[80%] h-full py-6 px-20'>
                <div className=''>
                    <p className='font-bold text-xl text-destructive'>{title}</p>
                    <p className='text-xs text-neutral-400 pt-2'>{heading}</p>
                    <p className='text-sm font-bold mt-5'>{projectContent}</p>
